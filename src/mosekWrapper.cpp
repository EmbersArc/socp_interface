...
Expression::t convert_affine_expression(
    const op::AffineExpression &expr,
    const vector<Variable::t> &mosek_variables)
{
    ...
    for (size_t i = 0; i < expr.terms.size(); ++i)
    {
        ...
        {
            (*exprs)[i] = Expr::mul(mosek_variables[affineTerm.variable.value().problem_index].get(), affineTerm.parameter.get_value());
        }
        else
        {
...
void MosekWrapper::solveProblem(bool verbose)
{
    Model::t M = new Model();
    // auto _M = finally([&]() { M->dispose(); });

    vector<Variable::t> mosek_variables;
    for (size_t i = 0; i < socp.getNumVariables(); ++i)
    {
    ...
    solution_vector.resize(socp.getNumVariables());
    for (size_t i = 0; i < socp.getNumVariables(); ++i)
    {
